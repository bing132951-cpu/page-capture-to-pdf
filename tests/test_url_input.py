...
class UrlInputTests(unittest.TestCase):
    ...
    def test_parse_urls_text_accepts_webp_and_png(self) -> None:
        document = parse_urls_text("https://example.com/1.webp\nhttps://example.com/2.png\n")
        self.assertEqual(document.total_pages, 2)
        self.assertEqual(document.pages[0].image_url, "https://example.com/1.webp")

    def test_parse_urls_text_rejects_unsupported_image_url(self) -> None:
        with self.assertRaises(UrlInputError) as error:
            parse_urls_text("https://example.com/image.gif")
        self.assertIn(".jpg/.jpeg/.png/.webp", str(error.exception))
